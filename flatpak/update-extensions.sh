#!/bin/bash
# Script to update the Flatpak extension sources from eclipse-theia/builtin-extension-pack
#
# Usage:
#   ./update-extensions.sh          # Update extension-sources.json
#   ./update-extensions.sh --check  # Check if extensions are up-to-date (for CI)
#
...
OUTPUT_FILE="${SCRIPT_DIR}/extension-sources.json"
CHECK_MODE=false
...
if [ "$CHECK_MODE" = true ]; then
  ...
  if [ ! -f "$OUTPUT_FILE" ]; then
    echo "ERROR: $OUTPUT_FILE does not exist. Run without --check first." >&2
    exit 1
  fi
  
  EXISTING=$(cat "$OUTPUT_FILE")
  
  if [ "$FINAL_JSON" = "$EXISTING" ]; then
    echo "Extension sources are up-to-date." >&2
    exit 0
  else
    echo "ERROR: Extension sources are out of date!" >&2
    echo "Run 'flatpak/update-extensions.sh' to update them." >&2
    exit 1
  fi
else
  # Write to file
