 ...
 *   --package             Package the electron application after building [default: false]
...
function buildIde() {
    ...
    run('yarn build:applications:dev', ROOT_DIR, 'Build electron application');
 ...
 */
function packageApp() {
    run('yarn package:applications', ROOT_DIR, 'Package electron application');
}
...
async function main() {
    ...
    if (!options.unlink && !options.package) {
        console.log(`
Next steps:
  - Run the IDE: yarn --cwd applications/electron start
...
`);
    ...
    if (options.package) {
        console.log('\nPackaged application is in: applications/electron/dist/');
    }
