...
- Optional: Announce the start of the Theia IDE Preview Test phase in your internal Slack channel:
   ...
   Please take a moment to test it and provide feedback - whether you've run into issues or everything works as expected. https://github.com/eclipse-theia/theia/discussions/{{discussionNumber}}
...
- Update the download links in the discussion's description:
...
- Update the discussion's status table with two rows for the current patch release:
   ...
   ```md
   | {{ideVersion}} preview available | {{previewStart}} | :white_check_mark: |
   | {{ideVersion}} community preview window | {{previewStart}} → {{previewEnd}} | :hourglass_flowing_sand: |
   ```
