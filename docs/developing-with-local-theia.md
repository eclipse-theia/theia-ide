...
4. Build the Theia IDE extensions and electron application
...
cd ../theia-ide && yarn build:applications:dev
```
...
The packaged application will be in `applications/electron/dist/`.
...
If you packaged the application with `--package`, you can also run the packaged version directly from `applications/electron/dist/`.
...
| `--package`           | Package the electron application after building      |
